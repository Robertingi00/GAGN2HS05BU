...
#### 1. Write the SQL code that will create the table structure for a table named EMP_1. This table is a subset of the EMPLOYEE table. The
basic EMP_1 table structure is summarized in the table below. (Note that the JOB_CODE is the FK to JOB.)
...
select PROJ_NUM_FK, ASSIGN_CHG_HR, ASSIGN_HOURS, sum(ASSIGN_CHG_HR*ASSIGN_HOURS) from ASSIGNMENT;

select PROJ_NUM_FK, ASSIGN_CHG_HR, ASSIGN_HOURS from ASSIGNMENT;
